...
export const styles = sortCx({
    ...
    colors: {
        primary: {
            root: [
                "bg-[var(--color-blue-400)] text-white shadow-xs-skeumorphic ring-1 ring-transparent ring-inset hover:border-color data-loading:bg-brand-solid_hover overflow-hidden",
        ...
        secondary: {
            root: [
                "bg-primary text-ai border-ai shadow-xs-skeumorphic ring-1 ring-primary ring-inset hover:bg-primary_hover hover:text-ai data-loading:bg-primary_hover overflow-hidden",
                // Disabled styles
        ...
        tertiary: {
            root: [
                "text-tertiary text-ai hover:bg-primary_hover hover:text-tertiary_hover data-loading:bg-primary_hover",
                // Icon styles
                "*:data-icon:text-fg-quaternary hover:*:data-icon:text-fg-quaternary_hover",
            ].join(" "),
        },
...
export const ButtonAI = ({
    ...
    return (
        ...
        >
            {/* Organic blob shapes for primary variant */}
            {color === "primary" && !disabled && (
                <>
                    {/* Blob 1 - Teal */}
                    <div className="absolute inset-0 opacity-80 pointer-events-none">
                        <div className="absolute w-16 h-16 bg-[var(--color-teal-400)] rounded-full animate-[blob1_6s_ease-in-out_infinite] blur-[7px]" />
                    </div>

                    {/* Blob 2 - Blue */}
                    <div className="absolute inset-0  pointer-events-none">
                        <div className="absolute w-12 h-12 bg-[var(--color-brand-500)] rounded-full animate-[blob2_4s_ease-in-out_infinite_reverse] blur-[12px]" />
                    </div>

                    {/* Blob 3 - Green */}
                    <div className="absolute inset-0 opacity-100 pointer-events-none">
                        <div className="absolute w-12 h-12 bg-[var(--color-success-500)] rounded-full animate-[blob3_7s_ease-in-out_infinite] blur-[12px]" />
                    </div>
                </>
            )}

            {/* Organic blob shapes for secondary variant */}
            {color === "secondary" && !disabled && (
                <>
                    {/* Blob 1 - Teal */}
                    <div className="absolute inset-0 opacity-40 pointer-events-none">
                        <div className="absolute w-16 h-16 bg-[var(--color-teal-300)] rounded-full animate-[blob1_6s_ease-in-out_infinite] blur-[7px]" />
                    </div>
                    ...
                    <div className="absolute inset-0 opacity-30 pointer-events-none">
                        <div className="absolute w-12 h-12 bg-[var(--color-success-400)] rounded-full animate-[blob3_7s_ease-in-out_infinite] blur-[12px]" />
                    </div>
                </>
            )}
...
{children && (
    <span
      data-text
      className={cx(
        "relative z-20 transition-inherit-all",
        !noTextPadding && "px-0.5",
        color === "secondary" && "text-ai" ,
        color === "tertiary" && "text-ai",
        // ← gradient only for secondary
      )}
    >
      {children}
    </span>
