...
export const AlertFloating = ({ title, description, confirmLabel, onClose, onConfirm, color = "default", dismissLabel = "Dismiss" }: AlertFloatingProps) => {
    const hasActions = onConfirm || (onClose && confirmLabel);

    return (
        <div className="relative flex flex-col gap-4 rounded-xl border border-primary bg-primary_alt p-4 shadow-xs md:flex-row w-fit">
            ...
            <div className="flex flex-1 flex-col gap-3">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-sm text-tertiary md:pr-0">{description}</p>
                </div>

                {hasActions && (
                    <div className="flex gap-3">
                        {onClose && confirmLabel && (
